...
const { customBol } = toRefs(keyBoardStore)
...
// 演示模式数据
import { getMockKeyboardLightList } from '../../../utils/demonstrationData.js'
// Hex转RGB
const hexToRgb = (hex) => {
...
const onLightingSettingChange = async () => {
    try {
        if (!isDemoMode.value) {
            ...
            // 同步当前选中的颜色到 store，供 interaction 组件使用
            keyBoardStore.currentPickColor = HexColorVal.value;
...
const currentLightingMode = computed(() => {
    const item = lightingModeOptions.value.find(item => item.ledIndex === lightingMode.value) || {};
    // 同步更新 store 中的 customBol (Side Effect)
    customBol.value = !!item.customBol;
    return item;
});
...
onMounted(async () => {
    ...
    try {
        if (isDemoMode.value) {
            ...
            lightingModeOptions.value = getMockKeyboardLightList();
